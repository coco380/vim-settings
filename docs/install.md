...
`~/.vimrc`、`~/.vimrc.minimal`、`~/.vim/coc-settings.json` は、このリポジトリの設定を Mac 本体へ反映する場合に上書き対象になります。既に存在する場合は、必ず backup を作ってからコピーします。

`~/.zshrc` は `lvim` / `vvim` alias を永続化する場合だけ変更します。`PATH` が既に通っていて CLI を追加するだけなら、`~/.zshrc` の変更は不要です。
...
Tokyo Night colorscheme plugin を入れる場合だけ作成するもの:

```txt
~/.vim/pack/vendor/start/tokyonight-vim/
```

`~/.vim/`、`~/.local/bin/`、`~/.config/`、`~/.cache/` のディレクトリ自体は、他の用途でも使われる可能性があるため丸ごと所有しません。この設定で作った中身だけを管理対象にします。
...
`vimrc` は Tokyo Night colorscheme plugin が入っていれば `colorscheme tokyonight` を読み込みます。未導入の場合も内蔵 fallback で起動できます。

Vim 標準 package 機能で入れる場合:

```sh
mkdir -p "$HOME/.vim/pack/vendor/start"
git clone https://github.com/ghifarit53/tokyonight-vim.git "$HOME/.vim/pack/vendor/start/tokyonight-vim"
```
